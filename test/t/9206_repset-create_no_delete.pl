...
my $cmd3 = qq($homedir1/nodectl spock repset-create --replicate_delete=False $repset $database);
print("cmd3 = $cmd3\n");
...
my $cmd5 = qq($homedir2/nodectl spock repset-create --replicate_delete=False $repset $database);
print("cmd5 = $cmd5\n");
...
my $cmd15 = qq($homedir2/$version/bin/psql -t -h 127.0.0.1 -p $port2 -d $database -c "SELECT * FROM spock.replication_set");
print("cmd15 = $cmd15\n");
