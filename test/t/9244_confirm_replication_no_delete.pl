...
# Connect to psql on node 1 and delete a record from 'foo':
#

my $cmd28 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port -d $database -c "DELETE FROM foo WHERE col1 = 10");
print("cmd28 = $cmd28\n");
...
print("We've just deleted col1 = 10 from the foo table on $n1 using $port\n");
...
print("If 10 is missing from our search string from $n2 listening on $port2 the cluster is replicating insert statements and this script 
	should exit.\n");

if(!(contains(@$stdout_buf29[0], "10")))
{
    exit(1);
}


# Connect to psql on node 2 and delete a record from 'foo':
#

my $cmd8 = qq($homedir2/$version/bin/psql -t -h 127.0.0.1 -p $port2 -d $database -c "DELETE FROM foo WHERE col1 = 4");
print("cmd8 = $cmd8\n");
...
print("We've just deleted col1 = 4 from the foo table on $n2 using $port2\n");
...
print("If 4 is not in our search string from $n1 listening on $port the cluster is replicating delete statements and this script 
        should exit with fail.\n");

# Test for the search_term in a buffer.

if (contains(@$stdout_buf39[0], "4"))
