...
my $spock = "3.1";
my $cluster = "demo";
my $repset = "demo-notruncate-repset";
my $n1 = "~/work/nodectl/test/pgedge/cluster/demo/n1";
my $n2 = "~/work/nodectl/test/pgedge/cluster/demo/n2";
my $schema = "public";
my $table_name="foo_no_truncate";
...
# Checking Replication --truncate=False
    ...
    print("TRUNCATE=FALSE REPLICATION CHECK\n");
    print ("-"x45,"\n");
  
   my $cmd6 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "TRUNCATE $table_name");
    print("cmd6 = $cmd6\n");
   ...
   print ("-"x100,"\n"); 
    print("TRUNCATE=FALSE REPLICATION CHECK IN NODE n1 \n");
     print ("-"x45,"\n");
  ...
  # Listing table contents of Port2 6433
   print("TRUNCATE=FALSE REPLICATION CHECK IN NODE n2\n");
    print ("-"x45,"\n");
...
if(!(contains(@$stdout_buf6[0], "TRUNCATE")))
{
    exit(1);
}
#==================================================================================================================================================================================
#Generating series in table of port 1 6432
 ...
 print("GENERATING SERIES IN TABLE IN n1");
   my $cmd01 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "INSERT INTO public.$table_name select generate_series(1,10)");
   print("cmd01 = $cmd01\n");
   my($success01, $error_message01, $full_buf01, $stdout_buf01, $stderr_buf01)= IPC::Cmd::run(command => $cmd01, verbose => 0);
 
   print("="x100,"\n");
   
   my $cmd02 = qq($homedir/$version/bin/psql  -h 127.0.0.1 -p $port1 -d $database -c "SELECT * FROM $table_name");
   print("cmd02 = $cmd02\n");
   my($success02, $error_message02, $full_buf02, $stdout_buf02, $stderr_buf02)= IPC::Cmd::run(command => $cmd02, verbose => 0);
   print("stdout_buf02= @$stdout_buf02\n");
  print("="x100,"\n");
 #===================================================================================================================================================================================
 #Checking Replication insert=True
    ...
    my $cmd12= qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "INSERT INTO $schema.$table_name values(555)");
    print("cmd12 = $cmd12\n");
