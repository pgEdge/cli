...
print("We just executed the command that creates the replication set (demo-noinsert-repset-n1)\n");


     # Creating foo Table

    my $cmd6 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "CREATE TABLE foo (col1 INT PRIMARY KEY)");
    print("cmd6 = $cmd6\n");
   ...
   my $cmd7 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "INSERT INTO foo select generate_series(1,10)");
   print("cmd7 = $cmd7\n");
