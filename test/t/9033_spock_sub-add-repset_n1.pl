...
# Add the repset to the subscriptions on node 1:
...
my $cmd24 = qq($homedir/nodectl spock sub-add-repset sub_n1n3 $repset $database);
print("cmd24 = $cmd24\n");
my($success24, $error_message24, $full_buf24, $stdout_buf24, $stderr_buf24)= IPC::Cmd::run(command => $cmd24, verbose => 0);

print("We just added the replication set to the subscription on ($n1) to connect to ($n3).\n");
