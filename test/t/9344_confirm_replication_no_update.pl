...
my $cmd28 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port -d $database -c "UPDATE foo SET col1 = '555' WHERE pkey = '777'");
print("cmd28 = $cmd28\n");
...
if(contains(@$stdout_buf29[0], "555"))
{
    exit(1);
}
...
my $cmd38 = qq($homedir2/$version/bin/psql -t -h 127.0.0.1 -p $port2 -d $database -c "UPDATE foo SET col1 = '222' WHERE pkey = '888'");
print("cmd38 = $cmd38\n");
...
if (contains(@$stdout_buf39[0], "222"))
