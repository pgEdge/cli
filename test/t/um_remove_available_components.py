...
for i, inner_list in enumerate(res): 
    ...
    remove_res=util_test.run_cmd("Removing components", remove_comp, f"{cluster_dir}/n1")

# UM List the components:
list=(f"um list")
list_res=util_test.run_cmd("Getting list of packages", list, f"{cluster_dir}/n1")
