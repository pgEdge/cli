...
# Checking Replication --insert=False
    ...
    print ("-"x45,"\n");
    my $cmd6= qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "DELETE FROM $schema.$table_name where col1=2");
    print("cmd6 = $cmd6\n");
...
if(!(contains(@$stdout_buf6[0], "DELETE")))
{
  ...
  #Checking Replication insert=True
    print("INSERT FUNCTION REPLICATION CHECK\n");
     print ("-"x45,"\n");
    my $cmd7 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "INSERT INTO $schema.$table_name values(888)");
    print("cmd7 = $cmd7\n");
