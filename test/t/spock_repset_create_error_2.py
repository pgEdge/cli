...
command = f"spock repset-create my_repset {dbname}"
...
command = f"spock repset-create my_repset {dbname}"
res2=util_test.run_cmd("Run spock sub-create -r.", command, f"{cluster_dir}/n1")
print(f"Print our command here: {command}")
print(f"Print res2.stdout here: - {res2.stdout}")
print("*"*100)

# Needle and Haystack
#Check needle and haystack with res.stdout:
check=util_test.contains(res2.stdout,"already exists")
print("*"*100)
