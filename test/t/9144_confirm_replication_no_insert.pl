...
my $cmd28 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port -d $database -c "INSERT INTO foo VALUES ('777')");
print("cmd28 = $cmd28\n");
...
if(contains(@$stdout_buf29[0], "777"))
{
    exit(1);
}
...
my $cmd38 = qq($homedir2/$version/bin/psql -t -h 127.0.0.1 -p $port2 -d $database -c "INSERT INTO foo VALUES ('888')");
print("cmd38 = $cmd38\n");
...
print("If 888 is in our search string from $n1 listening on $port the cluster is replicating insert statements and this script 
        should exit with fail.\n");

# Test for the search_term in a buffer.

if (contains(@$stdout_buf39[0], "888"))
