...
# ./nodectl install pgedge --pg 15 -U admin -P password -d demo
...
my $cmd = qq(./nodectl install pgedge --pg 15 -U $username -P $password -d $database);
print("cmd = $cmd\n");
...
my $cmd5 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port -U $username -d $database -c "select * from current_database()");
print("cmd5 = $cmd5\n");
...
my $cmd6 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port -U $username -d $database -c "SELECT installed_version FROM pg_available_extensions WHERE name='spock'");
print("cmd6 = $cmd6\n");
