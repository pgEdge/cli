...
# Checking Replication --update=False
    ...
    print("UPDATE=FALSE REPLICATION CHECK\n");
    print ("-"x45,"\n");
   my $cmd6 = qq($homedir/$version/bin/psql -t -h 127.0.0.1 -p $port1 -d $database -c "UPDATE $table_name SET col1=333 where col1=3");
    print("cmd6 = $cmd6\n");
   ...
   print ("-"x100,"\n"); 
    print("UPDATE=FALSE REPLICATION CHECK IN NODE n1 \n");
     print ("-"x45,"\n");
  ...
  # Listing table contents of Port2 6433
   print("UPDATE=FALSE REPLICATION CHECK IN NODE n2\n");
    print ("-"x45,"\n");
...
if(!(contains(@$stdout_buf6[0], "UPDATE")))
{
    exit(1);
}
