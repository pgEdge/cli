...
my $homedir3 = $out3->[0]->{"home"};
print("The home directory is {$homedir3}\n");

# We can retrieve the port number from nodectl in json form...
my $json4 = `$n3/pgedge/nc --json info $version`;
#print("my json = $json4");
my $out4 = decode_json($json4);
my $port3 = $out4->[0]->{"port"};
print("The port number is {$port3}\n");

my $cmd14 = qq($homedir3/$version/bin/pgbench -i --port $port3 $database);
...
my $cmd49 = qq($homedir3/$version/bin/psql -t -h 127.0.0.1 -p $port3 -d $database -c "select * from pgbench_accounts limit 5");
print("cmd49 = $cmd49\n");
...
my $cmd7 = qq($homedir3/$version/bin/psql -t -h 127.0.0.1 -p $port3 -d $database -c "SELECT * FROM pgbench_accounts ORDER BY aid LIMIT 10");
print("cmd7 = $cmd7\n");
