...
def get_cli_ver(p_file):
    ...
    awk3 = "awk '{print $3}'"
    ver_quoted = util.getoutput(f"grep 'MY_VERSION =' {p_file} | {awk3}")
...
def backup_current():
    ts = get_now()
    ver = get_cli_ver({MY_HOME}/hub/scripts/install.py)
...
def list_archives(base_dir, wildcard):
    dir_list = []

    for name in os.listdir(BASE_BACKUP_DIR):
        if ["_BACKUP", "_DOWNLOAD"] in name:
