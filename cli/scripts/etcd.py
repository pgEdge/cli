...
def etcd_conf(primary_node, nodes):
    primary_etcd_yaml = (
        ...
        .replace("IP_NODE", primary_node['local_ip'])
        .replace("INITIAL_CLUSTER", 
                 f"{primary_node['name']}=http://{primary_node['local_ip']}:2380")
        .replace("STATE", "new")
    )
    primary_cmd = (
    ...
    util.run_rcommand(
        ...
        host=primary_node["ip_address"], usr=primary_node["os_user"], 
    ...
    for node in nodes:
        initial_cluster_configs += (
            f",{node['name']}=http://{node['local_ip']}:2380"
        )
        etcd_yaml = (
            ETCD_YAML.replace("NODE_NAME", node['name'])
            .replace("IP_NODE", node['local_ip'])
        ...
        util.run_rcommand(
            ...
            host=node["ip_address"], usr=node["os_user"], 
            key=node["ssh_key"]
        )
...
def configure_etcd(primary_node, nodes, system_identifier):
    ...
    util.run_rcommand(
        f"{ETCDCTL} --endpoints=http://{primary_node['ip_address']}:2379 endpoint health",
    ...
    util.echo_cmd(
        ETCD_MEMBERS_LIST,
        host=primary_node["ip_address"],
        usr=primary_node["os_user"],
        key=primary_node["ssh_key"]
    )
    
    cmd = f"{ETCDCTL} --endpoints=http://{primary_node['ip_address']}:2379"
    for i, node in enumerate(nodes):
        cmd = cmd + ","
        cmd = cmd + f"http://{node['ip_address']}:2379"
