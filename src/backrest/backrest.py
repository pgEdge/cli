...
def write_config():
    ...
    with filename format <node_name>_<stanza>.yaml.
    """
...
def update_config(verbose=True):
    """
    Read the backrest YAML generated by write_config and
    apply each setting via util.set_value("BACKUP", key, value).
    """
    # find the YAML we wrote
    cfg        = fetch_config()
    stanza     = cfg.get("stanza", "stanza")
    node_name  = cfg.get("node_name", cfg.get("node", "node"))
    yaml_file  = os.path.join(os.getcwd(), "backrest", f"{node_name}_{stanza}.yaml")

    if not os.path.isfile(yaml_file):
        util.exit_message(f"Config file not found: {yaml_file}", level="error")
    ...
    for key, val in entries:
        ...
        try:
            ...
            util.exit_message(
                "util.set_value(section, key, value) is not implemented!",
                level="error"
            )

    util.message("✓ All BACKUP settings applied.")
    return True
